...
class BlogController  {
     ...
     getBlogs (req, res) {
          BlogModel.find({isDeleted : false,status : true}).populate({path : 'userId', select : 'name profile'}).exec(function(err, resData){
               if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
               if (resData) {
                    return res.json({resStatus:'success', msg :'Blog List', result : resData});
               } else {
     ...
     Description : use to update blog status
     --------------------------------------------------------------------------*/
     admin_status (req, res) {
          let blogId = req.query.blogId;
          let updatedStatus = req.query.status;
          BlogModel.findOneAndUpdate({_id:blogId},{'status':updatedStatus}, {new:true},function(err, resData){
               if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
               if (resData) {
                    return res.json({resStatus:'success', msg :'Blog status has been updated Successfully'});
               } else {
                    return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
               }
          });
