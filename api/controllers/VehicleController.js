     ...
     listStates (req, res) {
          ...
          **************************************************************************/
          ...
          admin_userCarlist (req, res) {
               VehicleModel.find({userId : req.query.userId}).populate({path:'userId', select: 'name  email'}).exec(function(err,resData){
                    if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
          ...
          admin_vehicleInfo (req, res) {
               VehicleModel.findOne({_id : req.query.vehicleId}).populate({path:'userId', select: 'name profile email'}).exec(function(err,resData){
                    if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
          ...
          admin_status (req, res) {
               ...
               VehicleModel.findOneAndUpdate({_id:vehicleId},{'status':updatedStatus}, {new:true},function(err, resData){
                    if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                    if (resData) {
                         let template = AppConstants.VEHICLE_STATUS;
                         template = template.replace("{{USERNAME}}", resData.userId.fullname);
                         let status = resData.status;
                         //console.log(resData)
                         if(status == true){
                              template = template.replace("{{STATUS}}", "Your Vehicle is varified");
                         }else{
                              template = template.replace("{{STATUS}}", "Your Vehicle has been bloked by corsa owner");
                         }
                         let mailOptions = {
          ...
          admin_updateVariant (req, res) {
               ...
               admin_updateCountry (req, res) {
                    ...
                    Description : use to get all vehicles of all Users
                    --------------------------------------------------------------------------*/
                    admin_vehicleList (req, res) {
                         VehicleModel.find({isDeleted : false }).populate({path:'userId', select: 'name profile'}).exec(function(err,resData){
                             if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                             return res.json({resStatus:'success', msg : 'vehicles List', result : resData});
                        });
                   }

                   admin_getVehicleBookingHistory (req,res) {
                       // let vehicleId = req.query.vehicleId;
                        BookingModel.find({
                            isDeleted : false
                        })
