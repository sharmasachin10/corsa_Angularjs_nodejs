          ...
          request.post(accessTokenUrl, { json: true, form: params }, function(err, response, token) {
               ...
               request.get({ url: peopleApiUrl, headers: headers, json: true }, function(err, response, profile) {
                    ...
                    CommonService.isGoogleIdExist(profile.sub, UserModel, function (emailStatus) {
                         console.log("lsdsdsd",emailStatus)
                              ...
                              if (userId) {
                                   ...
                                   CommonService.isEmailExist(profile.email, UserModel, function (emailExist) {
                                        if (emailExist == false) {
                                             ...
                                             newUser.fullname=newUser.name.first + " " + newUser.name.last;
                                        ...
                                        UserModel(newUser).save(function(err, resData) {
                                             //console.log(err)
                                        if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                                        let template = CommonService.getTemplate("SOCIAL-REGISTRATION-TEMPLATE",function(err,data){
                                             //console.log(data)
                                                       data.description = data.description.replace("{{USERNAME}}", newUser.fullname);
                                                       data.description = data.description.replace("{{PASSWORD}}", newUser.password);
                                                       let mailOptions = {
     ...
     facebook (req, res){
                         ...
                         if (emailStatus == false) {
                              if (userId) {
                                   UserModel.findOneAndUpdate({_id : userId },{facebookId : profile.id},{upsert : false, new : true}, function(err, updateData) {
                                        if(err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                                        return res.json({resStatus:'success', msg : "Facebook Account has been successfully linked", result : updateData});
                                   });
                              } else {
                                   ...
                                   CommonService.isEmailExist(profile.email, UserModel, function (emailExist) {
                                        ...
                                        let template = CommonService.getTemplate("SOCIAL-REGISTRATION-TEMPLATE",function(err,data){
                                             ...
                                             UserModel.findOneAndUpdate({email : profile.email },{facebookId : profile.id},{upsert : false, new : true}, function(err, updateData) {
                                                  if(err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                                                  return res.json({resStatus:'success', msg : "You have successfully logged in",token: JwtService.issueToken(updateData._id, req.body.platform, req.body.deviceToken), result : updateData});
                                             });
                                        }
                                   });
                              }
               ...
               if(reqData.email) {
                    CommonService.isEmailExist(reqData.email, UserModel, function (emailStatus) {
                         if (emailStatus === false) {
                              ...
                              // let template = AppConstants.SOCIAL_REGISTRATION_TEMPLATE;
                              // template = template.replace("{{PASSWORD}}", newUser.password);
                              // template = template.replace("{{USERNAME}}", newUser.fullname);
                              UserModel(newUser).save(function(err, resData) {
                                   if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                                   let template = CommonService.getTemplate("SOCIAL-REGISTRATION-TEMPLATE",function(err,data){
                                   data.description = data.description.replace("{{USERNAME}}", newUser.fullname);
          ...
          linkSocialAccount (req, res) {
               ...
               UserModel.findOneAndUpdate({_id : userId },updateCondn,{upsert : false}, function(err, updateData) {
                    console.log(updateData, "=== MY Resw");
                    if(err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
