...
angular.module('corsaAdminApp').controller('AddBrandController',['$LocalService','$scope', '$rootScope', '$state', '$AuthService', '$SessionService', '$location', '$localStorage','$AccountService','CountService','CarBrandService','$stateParams','addBrandService','NgTableParams','$window', function($LocalService, $scope, $rootScope, $state, $AuthService, $SessionService, $location, $localStorage,$AccountService,CountService,CarBrandService,$stateParams,addBrandService,NgTableParams,$window) {
...
/*  Confirm fron user to change Status of brand*/
 $scope.confirmBoxBrandr = function(brandId,stus) {
      var deleteUser = $window.confirm('Are you sure You want to Change Status  ?');
      //console.log(deleteUser);return;
      var brandId = brandId;
      var status = stus;
      if(deleteUser)
      $scope.updatedBrandStatus(brandId,status);
