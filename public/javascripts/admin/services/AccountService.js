...
angular.module('corsaAdminApp').factory('$AccountService', function($q,$http, $LocalService, AccessLevels, $localStorage,$SessionService) {
     return {
          ...
          changePassword: function(userId,new_password,con_password,cb){
               var userPass = $http.post('account/admin/updatePassword?reqId='+userId,
               {'new_password':new_password,'con_password':con_password});
          ...
          sendnewsletterEmail : function(message,email,cb){
               var msg = $http.post('newsletter/admin/sendEmailMsg',{'message':message,'email':email});
